...
class QuestionService:
    ...
    def get_questions(
        ...
        # Converte para QuestionResponse usando model_validate (Pydantic v2)
    ...
    def update_question(
        ...
        # LOG: mostra o que chegou
        print("="*50)
        print("update_data recebido:", update_data)
        print("reviewer_comments em update_data:", update_data.get('reviewer_comments'))
        ...
        # LOG: valor após setattr
        print("reviewer_comments após setattr:", question.reviewer_comments)

        db.commit()
        db.refresh(question)

        # LOG: valor após commit
        print("reviewer_comments após commit:", question.reviewer_comments)
        print("="*50)
    ...
    def approve_question(db: Session, question_id: int, current_user: User) -> Question:
        ...
        # Busca categoria "Aprovado"
        approved_category = db.query(Category).filter(Category.name == "Aprovado").first()
        if not approved_category:
            raise NotFoundException("Categoria 'Aprovado' não encontrada")
